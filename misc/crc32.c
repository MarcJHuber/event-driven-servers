 ...
 *
 * 32-bit CRC code, POSIX compliant
 *
 * $Id$
 *
 */
...
#if 0
/*
 * Code used for crctab_32 generation:
 */
#include <stdio.h>

int main()
{
    unsigned long long fcs, bit, gp, i;
    for (i = 0; i < 256;) {
	gp = 0x104a11db7 << 31, fcs = i << 32, bit = 1ULL << 63;
	for (; bit > 0xffffffff; bit >>= 1, gp >>= 1)
	    if (bit & fcs)
		fcs ^= gp;
	printf("0x%.8llx,%s", fcs, ++i % 6 ? " " : "\n");
    }
}
#endif
