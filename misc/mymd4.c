...
static void MD4Transform(u_int[4], u_char[64]);
...
#define GG(a, b, c, d, x, s) { \
    (a) += G ((b), (c), (d)) + (x) + (u_int)0x5a827999; \
    (a) = ROTATE_LEFT ((a), (s)); \
  }
#define HH(a, b, c, d, x, s) { \
    (a) += H ((b), (c), (d)) + (x) + (u_int)0x6ed9eba1; \
    (a) = ROTATE_LEFT ((a), (s)); \
  }

/* MD4 initialization. Begins an MD4 operation, writing a new context.
 */
void MD4Init(myMD4_CTX * context)
{
...
/* MD4 block update operation. Continues an MD4 message-digest
     ...
     context.
 */
void MD4Update(myMD4_CTX * context, u_char * input, u_int inputLen)
{
    u_int index;
    size_t i, partLen;
     ...
     */
    if (inputLen >= partLen) {
	memcpy((u_char *) & context->buffer[index], (u_char *) input, partLen);
    ...
    memcpy((u_char *) & context->buffer[index], (u_char *) & input[i], inputLen - i);
}

/* MD4 finalization. Ends an MD4 message-digest operation, writing the
     the message digest and zeroizing the context.
 */
void MD4Final(u_char digest[16], myMD4_CTX * context)
{
    u_char bits[8];
    u_int index, padLen;
     ...
     */
    memset((u_char *) context, 0, sizeof(*context));

}

/* MD4 basic transformation. Transforms state based on block.
 */
static void MD4Transform(u_int state[4], u_char block[64])
{
    ...
    /* Zeroize sensitive information.
     */
    memset((u_char *) x, 0, sizeof(x));
}

/* Encodes input (u_int) into output (u_char). Assumes len is
     a multiple of 4.
 */
static void Encode(u_char * output, u_int * input, u_int len)
{
    u_int i, j;
 ...
 */
static void Decode(u_int * output, u_char * input, u_int len)
{
    u_int i, j;
